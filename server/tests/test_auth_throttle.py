...
def test_repeated_failures_are_eventually_refused(client):
    ...
    responses = _fail_repeatedly(client, throttle.MAX_FAILURES + 1)
...
def test_the_right_password_is_refused_too_once_locked_out(client):
    """Otherwise the limit is trivially sidestepped by the attacker who just
    guessed correctly."""
    client.post("/auth/register", json=CREDENTIALS)
    client.post("/auth/logout")
    _fail_repeatedly(client, throttle.MAX_FAILURES)
...
def test_a_successful_sign_in_clears_the_count(client):
    ...
    client.post("/auth/register", json=CREDENTIALS)
    client.post("/auth/logout")
    _fail_repeatedly(client, throttle.MAX_FAILURES - 1)

    assert client.post("/auth/login", json=CREDENTIALS).status_code == 200
    client.post("/auth/logout")

    assert client.post("/auth/login", json=WRONG).status_code == 400


def test_the_limit_is_per_username(client):
    client.post("/auth/register", json=CREDENTIALS)
    client.post("/auth/logout")
    _fail_repeatedly(client, throttle.MAX_FAILURES)

    other = client.post("/auth/login", json={"username": "bob", "password": "wrong"})
    assert other.status_code == 400


def test_the_limit_follows_the_canonical_name(client):
...
def test_attempts_outside_the_window_are_forgotten(client, monkeypatch):
    client.post("/auth/register", json=CREDENTIALS)
    client.post("/auth/logout")
    _fail_repeatedly(client, throttle.MAX_FAILURES)
